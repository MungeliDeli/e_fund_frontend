...
const SectionEditor = ({ sections, onConfigChange }) => {
  ...
  return (
    <div className="space-y-2">
      {sections.map(section => (
        <div key={section.key} className="bg-[color:var(--color-background)] rounded-lg">
          <button
          ...
          {openSection === section.key && (
            <div className="p-4 border-t border-[color:var(--color-border)]">
              <SectionControls section={section} onConfigChange={onConfigChange} />
            </div>
          )}
        </div>
      ))}
