...
function ResetPasswordPage() {
  ...
  const validate = () => {
    const { error } = resetPasswordSchema.validate({ password, confirmPassword, token }, { abortEarly: false });
    if (!error) return {};
    const fieldErrors = {};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/reset-password', { resetToken: token, newPassword: password });
