...
function CoverImage({ mediaId, alt = "Cover image", className = "" }) {
  ...
  if (isLoading) {
    return <div className={`w-full h-48 bg-gray-300 dark:bg-gray-700 animate-pulse ${className}`} />;
  }

  if (isError || imageLoadError || !imageUrl) {
    return <div className={`w-full h-48 bg-[color:var(--color-muted)] ${className}`} />;
  }
