 ...
 *
 * Displays campaign posts created by the organizer.
 * Shows posts with type 'campaign' using the same PostCard component as the feed.
 *
 * Props:
...
import { getOrganizerCampaignPosts } from "../../../../feed/services/feedApi";
...
function CampaignsTab({ organizerId }) {
  ...
  const {
    ...
    queryKey: ["organizerCampaignPosts", organizerId],
    queryFn: async () => {
      if (!organizerId) return [];
      const data = await getOrganizerCampaignPosts(organizerId);
      return data;
    },
    enabled: !!organizerId,
  });
      ...
      <div className="w-full flex flex-col items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-[color:var(--color-secondary-text)] mb-6">
            This organizer hasn't created any campaign posts yet.
          </p>
        </div>
      </div>
    );
