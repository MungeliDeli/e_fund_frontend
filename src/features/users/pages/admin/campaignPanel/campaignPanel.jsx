...
import { TotalStatsCard, StatusStatsCard } from '../../../../../components/StatsCards';
import { FiFilter, FiLayers, FiFlag, FiCheckCircle, FiClock, FiXCircle, FiPlayCircle, FiCheckSquare, FiEdit2, FiPauseCircle, FiStopCircle } from 'react-icons/fi';
import CampaignTable from '../../../components/CampaignTable';

function CampaignPanel() {
  const [isFilterModalOpen, setIsFilterModalOpen] = useState(false);
  const [filters, setFilters] = useState({});
  // Placeholder for all campaigns data
  ...
  const handleCategories = () => {
    // TODO: Open categories modal or navigate to categories page
  };
  ...
  const campaignStatuses = [
    ...
    { key: 'approved', label: 'Approved', icon: FiCheckCircle, color: '#43e97b' },
    { key: 'active', label: 'Active', icon: FiPlayCircle, color: '#3b82f6' },
    { key: 'successful', label: 'Successful', icon: FiCheckSquare, color: '#10b981' },
    { key: 'closed', label: 'Closed', icon: FiStopCircle, color: '#ef4444' },
    { key: 'cancelled', label: 'Cancelled', icon: FiXCircle, color: '#ef4444' },
    { key: 'completed', label: 'Completed', icon: FiCheckSquare, color: '#38f9d7' },
  ...
  const valueMap = campaignStatuses.reduce((acc, s) => {
    acc[s.key] = allCampaigns.filter(c => (c.status || '').toLowerCase() === s.key).length;
    return acc;
  }, {});
  ...
  return (
    <div className="p-2 sm:p-2 bg-[color:var(--color-background)] min-h-screen transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 w-full">
        {/* Title */}
        <h1 className="text-3xl font-bold text-[color:var(--color-primary-text)]">Campaigns</h1>
        {/* SearchBar */}
      ...
      <div className="bg-[color:var(--color-background)] p-2 rounded-lg border border-[color:var(--color-muted)] shadow-md min-h-[120px]">
        <CampaignTable data={allCampaigns} onView={handleView} filters={filters} />
      </div>
