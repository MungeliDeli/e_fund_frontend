...
export function TotalStatsCard({
  ...
  return (
    <div
      className={`flex flex-col gap-1 bg-[color:var(--color-surface)] rounded-xl shadow p-6 min-h-[140px] flex-1 border border-[color:var(--color-muted)] ${className}`}
    >
      <div className="text-[color:var(--color-secondary-text)] text-md font-bold mb-3">
        {title}
      </div>
      ...
      <div className="flex items-center gap-4 mt-6">
        ...
        <span className="text-4xl font-bold text-[color:var(--color-primary-text)]">
          {value}
        </span>
      </div>
    </div>
...
export function PieStatsCard({
  ...
  return (
    <div
      className={`flex bg-[color:var(--color-surface)] rounded-xl shadow p-6 min-h-[140px] flex-1 border border-[color:var(--color-muted)] ${className}`}
    >
      {/* Left: Stats */}
      <div className="flex flex-col justify-between flex-1 pr-4">
        <div className="flex items-center gap-2 mb-2">
          {Icon1 && (
            <span
              ...
              style={{ background: color1 + "22" }}
            >
              <Icon1 className="text-lg" style={{ color: color1 }} />
            </span>
          )}
          <span className="text-md text-[color:var(--color-secondary-text)] font-bold">
            {title1}
          </span>
          <span className="text-2xl font-bold pl-2 text-[color:var(--color-primary-text)] mb-2">
            {value1}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <span className="text-2xl font-bold pl-2 text-[color:var(--color-primary-text)]">
            {value2}
          </span>
        </div>
      </div>
...
export function StatusStatsCard({
  ...
  return (
    <div
      className={`relative flex flex-col gap-4 bg-[color:var(--color-surface)] rounded-xl shadow p-6 min-h-[140px] flex-1 border border-[color:var(--color-muted)] ${className}`}
    >
        ...
        {open && (
          <div className="absolute right-0 top-10 z-10 bg-[color:var(--color-surface)] border border-[color:var(--color-muted)] rounded shadow-lg min-w-[160px]">
            {statuses.map((status) => (
              <div
                key={status.key}
                className={`flex items-center gap-2 px-4 py-2 cursor-pointer hover:bg-[color:var(--color-muted)] ${
                  selected.key === status.key ? "font-bold" : ""
                }`}
