...
function Header({ onMenuClick, className = "", user: userProp, isAuthenticated: isAuthenticatedProp }) {
  const { dark, toggleTheme } = useTheme();
  const navigate = useNavigate();
  const location = useLocation();
  const { user: userCtx, isAuthenticated: isAuthCtx, logout } = useAuth();
  const user = userProp || userCtx;
  const isAuthenticated = typeof isAuthenticatedProp === 'boolean' ? isAuthenticatedProp : isAuthCtx;
  ...
  const renderProfileMenu = () => (
    <div className="absolute right-0 mt-2 w-56 bg-[color:var(--color-surface)]  rounded shadow-lg z-[9999]" style={{display: profileOpen ? 'block' : 'none'}}>
      <button
        className="w-full text-left px-4 py-2 text-sm  flex items-center gap-2"
        onClick={() => { navigate('/profile-view'); setProfileOpen(false); }}
      >
  ...
  return (
    <header className={`w-full flex items-center gap-2 justify-between py-2 bg-[color:var(--color-background)] border-b border-[color:var(--color-muted)] relative ${className}`}>
      <button
      ...
      <div className="flex items-center gap-1 shrink-0">
        {isAuthenticated ? (
          <>
            ...
            {/* Notification icon */}
            <button className="p-2 rounded hover:bg-[color:var(--color-muted)] focus:outline-none relative" aria-label="Notifications">
